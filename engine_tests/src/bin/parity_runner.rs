...
fn main() {
    ...
    let output_dir = args.get(1).map(PathBuf::from).unwrap_or_else(|| PathBuf::from("test-reports"));
    ...
    F: FnOnce() -> Result<(), String>,
{
    ...
    let mut test = TestResult::new(id, name, category)
...
fn run_steam_id_tests(mut builder: ReportBuilder) -> ReportBuilder {
    ...
    ).with_priority(TestPriority::Critical).with_doc_reference(DOC_URL));
    ...
    builder = builder.add(run_test(
        ...
        || {
            ...
            let parsed = SteamId::parse_steam3(&steam3)
                .ok_or("Failed to parse SteamID3")?;
            if parsed.account_id() != 52079950 {
                return Err("SteamID3 roundtrip failed".to_string());
            }
            Ok(())
        },
    ).with_priority(TestPriority::High).with_doc_reference(DOC_URL));
    ...
    builder = builder.add(run_test(
        ...
        || {
            ...
            let parsed = SteamId::parse_steam2(&steam2)
                .ok_or("Failed to parse SteamID2")?;
            if parsed.account_id() != 52079950 {
    ...
    builder = builder.add(run_test(
        ...
        || {
            ...
            let gs = SteamId::from_parts(12345, 1, AccountType::GameServer, 
...
fn run_auth_tests(mut builder: ReportBuilder) -> ReportBuilder {
    ...
    use engine_shared::auth::{AuthSessionResponse, AuthSessionState, AuthSession, MockAuthProvider};
...
fn run_lobby_tests(mut builder: ReportBuilder) -> ReportBuilder {
    ...
    use engine_shared::lobby::{LobbyManager, LobbyType, LobbyError, LobbySearchFilter};
    ...
    builder = builder.add(run_test(
        ...
        || {
            ...
            let lobby_id = manager.create_lobby(owner, LobbyType::Public, 8);
            let lobby = manager.get_lobby_mut(lobby_id).ok_or("Lobby not found".to_string())?;
    ...
    builder = builder.add(run_test(
        ...
        || {
            ...
            lobby.add_member(SteamId::from_account_id(2)).map_err(|e| format!("{:?}", e))?;
...
fn run_chat_tests(mut builder: ReportBuilder) -> ReportBuilder {
    ...
    use engine_shared::chat::{ChatManager, ChatChannel, ChatResult, MAX_MESSAGE_LENGTH};
    ...
    builder = builder.add(run_test(
        ...
        || {
            ...
            let recipients = manager.send_message(sender, "Player1", ChatChannel::Global, "Hello!")
                .map_err(|e| format!("{:?}", e))?;
    ...
    builder = builder.add(run_test(
        ...
        || {
            ...
            manager.get_player_mut(receiver).ok_or("Player not found".to_string())?.mute_player(sender);
...
fn run_gsi_tests(mut builder: ReportBuilder) -> ReportBuilder {
    ...
    use engine_shared::gsi::{GsiProvider, GsiPayload, GsiMap, GsiPlayer, GsiReceiver, GsiError, GameMode, PlayerTeam};
    ...
    builder = builder.add(run_test(
        ...
        || {
            let provider = GsiProvider::new("Counter-Strike 2", 730, 14000, SteamId::from_account_id(12345));
    ...
    builder = builder.add(run_test(
        ...
        || {
            ...
            payload.player = Some(GsiPlayer::new(SteamId::from_account_id(123), "Player1", PlayerTeam::CT));
...
fn run_network_tests(mut builder: ReportBuilder) -> ReportBuilder {
    ...
    use engine_shared::net::{NetMsg, encode_to_bytes, decode_from_bytes, PROTOCOL_VERSION};
    ...
    builder = builder.add(run_test(
        ...
        || {
            ...
            let welcome = NetMsg::Welcome { client_id: ClientId(42) };
    ...
    builder = builder.add(run_test(
        ...
        || {
            ...
            match decoded {
                NetMsg::MapInfo(info) if info.name == "de_dust2" && info.crc == 0xDEADBEEF => Ok(()),
